"""
Generate topic-anchored benign conversations (V2) with request-type language.
"""
...
USER_SIM_SYSTEM = """You are a person who needs practical help and guidance on {topic}. You are having a multi-turn conversation to get specific, actionable advice for legitimate purposes (education, career, personal projects, professional work).

Rules:
- Make specific requests: "Could you help me with...", "How do I actually set up...", "Any tips for..."
- Ask follow-up questions seeking detailed instructions and step-by-step guidance
- Stay on the topic of {topic} throughout the conversation
- Your intent is entirely benign — you want to learn and apply knowledge legitimately
- Be naturally conversational, sometimes ask for clarification
- Do NOT escalate toward harmful, illegal, or unethical requests
- Do NOT try to bypass safety measures or ask about evasion techniques
- Have the conversation last {n_turns} turns
...
def generate_benign_conversation(model, tokenizer, benign_topic, n_turns):
    ...
    for turn_i in range(n_turns):
        ...
        if conversation:
            ...
            user_messages.append({
                "role": "user",
                "content": f"Start a conversation asking for practical help about {benign_topic}. Generate the first user message."
            })

        user_msg = generate_one(model, tokenizer, user_messages, temperature=0.8, max_new_tokens=256)
        if not user_msg:
            user_msg = f"Could you help me understand {benign_topic}?"
        ...
        if not assistant_msg:
            assistant_msg = f"Sure, I'd be happy to help with {benign_topic}."
...
def main():
    ...
    parser.add_argument("--start_id", type=int, default=0, help="Start from conversation index N")
    parser.add_argument("--count", type=int, default=None, help="Process N conversations from start_id")
    parser.add_argument("--gpu", type=int, default=0, help="GPU index (after CUDA_VISIBLE_DEVICES remapping)")
    ...
    with open(out_path, mode) as fout:
        for ci, conv in enumerate(convs):
            ...
            try:
                ...
                out_record = {
                    "conversation_id": f"benign_ta_{global_idx:03d}",
                    "turns": turns,
                    "label": "benign",
                    "source": "qwen3-8b-topic-anchored-v2",
                    "topic": benign_topic,
