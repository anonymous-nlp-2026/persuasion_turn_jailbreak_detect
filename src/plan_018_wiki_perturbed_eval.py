...
def main():
    print("Loading Wiki MLM encoder (plan_018)...")
    mlm_path = PROJ / "checkpoints/plan_018_wiki_mlm/best"
    encoder = load_mlm_encoder(mlm_path)
    tokenizer = AutoTokenizer.from_pretrained(mlm_path)

    print("Loading BiGRU head (plan_018)...")
    embed_dim = encoder.config.hidden_size
    gru = GRUClassifier(input_dim=embed_dim, hidden_dim=GRU_HIDDEN, num_layers=GRU_LAYERS, dropout=GRU_DROPOUT)
    gru_ckpt = torch.load(PROJ / "checkpoints/plan_018_wiki_mlm/gru/best_gru.pt", map_location=DEVICE, weights_only=True)
    ...
    plan004_ref = {
        ...
        "jb_mlm": {"k=1": -0.0256, "k=2": 0.0, "k=3": 0.0, "k=5": 0.0, "full": -0.026},
    }

    print("\n" + "=" * 80)
    print("COMPARISON: Wiki MLM (018) vs all models (F1 delta: clean - perturbed)")
    print("=" * 80)
    print(f"{'K':<6} {'Wiki MLM':>10} {'JB MLM':>10} {'Treatment':>10} {'Baseline':>10} {'TF-IDF':>10}")
    print("-" * 58)
    for k_label in ["k=1", "k=2", "k=3", "full"]:
        wiki_d = -results["delta"].get(k_label, 0)
        jb_d = plan004_ref["jb_mlm"].get(k_label, float("nan"))
        ...
        print(f"{k_label:<6} {wiki_d:>+10.4f} {jb_d:>+10.4f} {tr_d:>+10.4f} {bl_d:>+10.4f} {tf_d:>+10.4f}")

    out = {
        "experiment": "plan_018_wiki_perturbation",
        "description": "Wikipedia MLM control model evaluated on adversarially perturbed data",
    ...
    out_path = PROJ / "results/plan_018_wiki_perturbation.json"
