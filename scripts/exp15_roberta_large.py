"""Exp15: RoBERTa-large (355M) DAPT + evaluation pipeline.
...
def train_dapt(seed, device):
    ...
    save_dir = CKPT_ROOT / f"exp15_roberta_large_9class_seed{seed}" / "deberta_multitask" / "best"
...
def main():
    ...
    print("=" * 60)
    print("Exp15: RoBERTa-large (355M) DAPT + Evaluation")
    print("=" * 60)
    ...
    for variant in ["vanilla", "9class"]:
        ...
        for seed in SEEDS:
            ...
            if variant == "vanilla":
                ...
                gru_save_dir = CKPT_ROOT / f"exp15_roberta_large_vanilla_seed{seed}" / "gru"
            else:
                ...
                del model
                gru_save_dir = CKPT_ROOT / f"exp15_roberta_large_9class_seed{seed}" / "gru" / "treatment"
    ...
    # Save results
    out_path = PROJ / "results/exp15_roberta_large.json"
