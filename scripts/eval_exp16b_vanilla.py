"""Evaluate exp16b Llama-only vanilla baseline across multiple test sets."""
import os
...
GRU_CKPT = PROJ / "checkpoints/exp16b/vanilla_seed42/baseline/best.pt"
...
def main():
    ...
    print(f"Loading DeBERTa: {ENCODER_NAME}")
    ...
    with open(out_dir / "exp16b_vanilla_results.json", "w") as f:
        json.dump(results, f, indent=2)

    print(f"\nResults saved to {out_dir / 'exp16b_vanilla_results.json'}")

    print("\n" + "="*60)
    print("SUMMARY: exp16b Llama-only Vanilla Baseline (seed42)")
    print("="*60)
