# Cross-LLM evaluation: test DeBERTa+GRU detectors on Llama-generated data
import os
...
def load_cross_llm_data(jailbreak_path, benign_path):
    conversations = []
...
def main():
    ...
    parser.add_argument("--output", type=str,
                        default="./results/cross_llm_eval.json")
    args = parser.parse_args()

    device = torch.device(f"cuda:{args.gpu}")
    benign_path = f"{DATA_ROOT}/benign_llama.jsonl"
    ...
    all_results = {"generation_model": "llama3.1-8b", "attack_results": {}}

    for attack_name, attack_path in attack_configs.items():
        print(f"\n{'='*60}")
        print(f"Attack type: {attack_name.upper()}")
        print(f"{'='*60}")

        conversations = load_cross_llm_data(attack_path, benign_path)
        n_jb = sum(1 for c in conversations if c["label"] == 1)
        n_bn = sum(1 for c in conversations if c["label"] == 0)
        print(f"  {n_jb} jailbreak + {n_bn} benign = {len(conversations)} total")
    ...
    print("\n" + "="*80)
    print("SUMMARY: Cross-LLM Evaluation (Llama 3.1-8B data, Macro F1, mean +/- std)")
    print("="*80)
    header = f"{'Variant':<10}"
