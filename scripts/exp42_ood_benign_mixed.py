#!/usr/bin/env python3
"""exp42: OOD benign mixed training — add WildChat benign to training data.

Hypothesis: Including WildChat benign conversations in training data will reduce
WildChat FPR from 68.1% to below 30% while maintaining jailbreak OOD detection
F1 above 0.95 on DD/AA/FITD.

Run: source ~/miniconda3/etc/profile.d/conda.sh && conda activate base && \
     cd . && \
     CUDA_VISIBLE_DEVICES=0 python scripts/exp42_ood_benign_mixed.py
"""
...
MIXED_TRAIN_PATH = PROJ / "data" / "exp42_mixed_train.jsonl"
WILDCHAT_EVAL_PATH = PROJ / "data" / "exp42_wildchat_eval.jsonl"

RESULTS_JSON = PROJ / "results" / "exp42_ood_benign_mixed.json"

# === Jailbreak filter keywords (from prepare_wildchat_benign.py) ===
JAILBREAK_KEYWORDS = [
...
def filter_wildchat(conversations):
    """Filter out likely jailbreak content from WildChat benign data."""
    clean = []
    filtered_ids = []
...
def prepare_wildchat_for_training(conv):
    """Add persuasion_strategy=0 to user turns for TurnDataset compatibility."""
    new_turns = []
...
def prepare_data():
    ...
    n_train = 150
    if len(wildchat_clean) < n_train + 20:
        n_train = int(len(wildchat_clean) * 0.65)

    wc_train = wildchat_clean[:n_train]
    wc_eval = wildchat_clean[n_train:]
    print(f"Split: {len(wc_train)} train, {len(wc_eval)} eval")
    ...
    labels_orig = Counter(c["label"] for c in original_train)
    print(f"  Label distribution: {dict(labels_orig)}")

    mixed_train = original_train + wc_train_formatted
    ...
    print(f"Saved mixed train to {MIXED_TRAIN_PATH}")
...
def evaluate_all(seeds_ckpts):
    ...
    for seed, (deberta_path, gru_path) in seeds_ckpts.items():
        ...
        # WildChat FPR (all should be benign=0, any predicted as 1 is FP)
        ...
        for ood_name, ood_data in ood_datasets.items():
            # OOD data is all jailbreak — combine with IID benign test for F1 calc
            ood_preds, _ = predict_convs(ood_data)
            ood_true = [1] * len(ood_data)  # all jailbreak
            # Recall on jailbreak OOD
            ...
            # F1: combine OOD jailbreak with IID benign test subset
...
if __name__ == "__main__":
    ...
    print(f"\nCheckpoints at: {CKPT_BASE}/exp42_mixed_seed*/")
