#!/bin/bash
# exp17 GPU 3 resume: 9-class seed42 stage2 + seeds 123,456 full
set -e
set -o pipefail
...
echo "=== exp17 GPU3 Resume (9-class): $(date) ==="

# seed42: stage1 done, only stage2
echo ""
echo ">>> 9-class Stage2 (GRU) seed=42 — $(date)"
python src/train_classifier.py \
    --train_data "$DATA_DIR/train.jsonl" \
    --val_data "$DATA_DIR/val.jsonl" \
    --mode treatment \
    --deberta_checkpoint "$CKPT_DIR/9class_seed42/deberta_multitask/best" \
    --model_name microsoft/deberta-v3-base \
    --output_dir "$CKPT_DIR/9class_seed42" \
    --epochs 20 --batch_size 32 --hidden_dim 256 --num_layers 2 \
    --dropout 0.3 --lr 1e-3 --max_length 256 --seed 42

# seeds 123, 456: full (stage1 + stage2)
