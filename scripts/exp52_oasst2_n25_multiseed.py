#!/usr/bin/env python3
"""exp52: OASST2 N=25 Multi-seed Validation.

Runs OASST2 benign adaptation with parameterized seed and n_adapt.
Reuses exp51 logic with CLI arguments for seed/n_adapt.
"""

import os
os.environ["CUDA_VISIBLE_DEVICES"] = "0"

import sys, json, random, time, argparse, subprocess
import numpy as np
import torch
...
def run_adaptation(local_model_path, oasst2_adapt, n_adapt, seed):
    print(f"\n{'=' * 70}", flush=True)
    print(f"Adaptation N={n_adapt}, seed={seed}", flush=True)
    print("=" * 70, flush=True)
    ...
    mixed_path = PROJ / "data" / f"exp52_mixed_train_n{n_adapt}_seed{seed}.jsonl"
    save_jsonl(mixed_train, mixed_path)

    name = f"exp52_oasst2_n{n_adapt}_seed{seed}"
    ...
    if result.returncode != 0:
        ...
        raise RuntimeError(f"Stage 1 failed for N={n_adapt} seed={seed}")

    deberta_path = ckpt_dir / "best"
    assert (deberta_path / "model.pt").exists(), f"No model.pt at {deberta_path}"
...
def main():
    ...
    parser.add_argument("--n_adapt", type=int, default=25)
    args = parser.parse_args()

    seed = args.seed
    n_adapt = args.n_adapt

    results_path = PROJ / "results" / f"exp52_seed{seed}.json"

    t_total = time.time()
    print("=" * 70, flush=True)
    print(f"exp52: OASST2 N={n_adapt} Multi-seed (seed={seed})", flush=True)
    print("=" * 70, flush=True)
    ...
    assert n_adapt <= len(oasst2_adapt), f"n_adapt={n_adapt} > pool size {len(oasst2_adapt)}"
    ...
    deberta_path, gru_path = run_adaptation(local_model_path, oasst2_adapt, n_adapt, seed)
    ...
    print(f"\n{'=' * 70}", flush=True)
    print(f"RESULTS: seed={seed}, N={n_adapt}", flush=True)
    print(f"{'=' * 70}", flush=True)
    print(f"  OASST2 FPR: {eval_results['oasst2_fpr']:.4f}", flush=True)
    print(f"  IID F1:     {eval_results['iid_f1_macro']:.4f}", flush=True)
    for k in ["dd", "aa", "fitd"]:
        print(f"  OOD {k.upper()} F1:  {eval_results.get(f'ood_{k}_f1', 'N/A')}", flush=True)
